...
def main():
    ...
    log("-" * 40)
    log("📊 成功率统计:")
    log(f"  页面: {STATS.pages_ok}/{total_pages} ({page_rate:.1f}%)")
    log(f"  资源: {STATS.assets_ok}/{total_assets} ({asset_rate:.1f}%)")
    log(f"  总体: {overall_rate:.1f}%")
    log(f"  总大小: {STATS.bytes/1024/1024:.2f} MB")
    if enc_result:
        log("-" * 40)
        log("🔐 加密版 (需密码才能浏览):")
...
if __name__ == "__main__":
    # V8.0: 启动时打印版本横幅
    print("=" * 60)
    print(f"       网站克隆器 {VERSION} 超级版  Website Cloner")
    print(f"  ★ 10次重试 ★ 失败回刷 ★ 加密保护 ★ 自动恢复 ★ 成功率统计")
    print("=" * 60)
    main_with_guard()
