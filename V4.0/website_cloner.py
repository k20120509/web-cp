...
class WebsiteCloner:
    ...
    def clone_page(self, url, depth):
        ...
        self._rewrite_resources(soup, final, base_href, extra_urls)
        nxt = self._rewrite_links(soup, final, base_href, depth)
        for b in soup.find_all("base"):
            b.decompose()

        local = self.page_path(final if final != url else url)
        local.parent.mkdir(parents=True, exist_ok=True)
    ...
    def _rewrite_resources(self, soup, page_url, base_href, extra_urls=None):
        tasks = []
        for tag, attr in RESOURCE_SPECS:
            for el in soup.find_all(tag):
                v = el.get(attr)
        ...
        for st in soup.find_all("style"):
            if st.string:
                new = self._rewrite_css_text(st.string, page_url, base_href)
                if new is not None:
    ...
    def _rewrite_links(self, soup, page_url, base_href, depth):
        ...
        for a in soup.find_all("a", href=True):
            ...
            if not is_page(absu):
                ...
                a["href"] = self.rel(self.page_path(page_url), self.out_root / rp) if ok else absu
                continue
            if self.max_depth == 0:
            ...
            a["href"] = self.rel(self.page_path(page_url), self.page_path(norm))
