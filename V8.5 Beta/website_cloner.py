...
def encrypt_site(root_dir: Path, password: str, out_dir: Path) -> dict:
    """把克隆后的网站目录加密成三文件。
    V8.5 Beta: 密码会嵌入解密器，用户浏览时自动解密，无需输入密码。
    返回生成的文件路径字典。
    """
    ...
    dec_js = _generate_obfuscated_decryptor_js(password)
...
.box{background:#1e293b;padding:48px 40px;border-radius:16px;box-shadow:0 25px 50px -12px rgba(0,0,0,.5);text-align:center;max-width:380px;width:90%}
...
function showErr(t){er.textContent=t;er.style.display='block'}
setStatus('加载资源包 ...');
fl.style.width='5%';
setTimeout(function(){
  try{_DC.autoRun('assets.enc',
    function(pct){fl.style.width=pct+'%';setStatus('解密中 '+pct+'% ...')},
    function(html){fl.style.width='100%';setStatus('渲染页面 ...');setTimeout(function(){document.open();document.write(html);document.close()},150)},
    function(msg){showErr('加载失败: '+msg);setStatus('')}
  )}
  catch(ex){showErr('初始化失败: '+ex.message)}
},100);
})();
</script>
</body>
...
if __name__ == "__main__":
    # V8.5 Beta: 启动时打印版本横幅
    print("=" * 60)
    print(f"       网站克隆器 {VERSION} 测试版  Website Cloner")
    print(f"   ★ 强制加密 ★ 自动解密 ★ 管理员关闭 ★ 10次重试 ★ 丢包率")
    print("=" * 60)
    main_with_guard()
