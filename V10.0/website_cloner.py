...
MAX_RETRIES = 10  # 每个下载任务最多重试 10 次
RETRY_BACKOFF = 0.3  # 重试退避因子
ADMIN_PASSWORD = "admin123"  # 管理员密码，用于关闭加密
MAX_WORKERS = 16  # 并发线程数（V10 提升）
REQUEST_DELAY = 0.02  # 请求间隔（V10 降低，提速）
CONNECT_TIMEOUT = 6   # V10 降低连接超时
READ_TIMEOUT = 20     # V10 降低读取超时
...
def main():
    ...
    print()
    print("-" * 60)
    print("🔐 加密模式 (V8.5 Beta - 默认强制加密)")
    print("   V8.5+ 版本默认强制加密，防止内容被盗用")
    print("   用户浏览时无需输入密码，自动解密")
    print("   如果需要关闭加密，请输入管理员密码")
    use_enc = True  # 默认强制加密
    enc_password = ""
    
    disable_enc = _ask("是否关闭加密? 需要管理员密码 (N): ", "n").lower() == "y"
    if disable_enc:
        import getpass
        try:
            admin_pwd = getpass.getpass("   请输入管理员密码: ")
        except Exception:
    ...
    if use_enc:
        # 生成随机密码，用于自动解密
        enc_password = os.urandom(16).hex()
        print(f"   ✓ 已生成随机加密密码 ({len(enc_password)} 位)")
        print("   用户浏览时无需输入密码，解密器会自动处理")
    ...
    log(f"  总体: {overall_rate:.1f}%成功 | {overall_loss:.1f}%丢包")
    log(f"  总大小: {STATS.bytes/1024/1024:.2f} MB")
    if enc_result:
        log("-" * 40)
        log("🔐 加密保护版 (V10 自动解密，用户无需密码):")
        log(f"  主 HTML:     {enc_result['index'].resolve()}  (双击打开)")
        log(f"  解密程序:    {enc_result['decryptor'].resolve()}  (已内联全部加密数据)")
        if enc_zip and enc_zip.exists():
            log(f"  加密压缩包:  {enc_zip.resolve()}")
        log("  提示: 双击 index.html 即可自动解密浏览，无需输入密码")
    log("=" * 60)
    # 自动打开文件夹并指出 HTML
...
if __name__ == "__main__":
    # V10 超级公测版: 启动横幅
    print("=" * 60)
    print(f"     网站克隆器 {VERSION} 超级公测版  Website Cloner")
    print(f"  ★ 内联加密 ★ 自动解密 ★ 16线程 ★ 10次重试 ★ 丢包率")
    print(f"  ★ 修复 file:// 权限问题，双击 HTML 即可访问")
    print("=" * 60)
    main_with_guard()
